...
#C U C^\dagger = U^\dagger; and C U^\dagger C^\dagger = U; and C E C^\dagger = -E.
...
function chess_operator_squared(N::Int)
    sp = [0. 1.; 0. 0.]
    sm = [0. 0.; 1. 0.]
    sz = [1. 0.; 0. -1.]
    u = [1. 0.; 0. 1.]
    ...
    Meven[1,:,4,:] = operator_5_dag
    Modd[4,:,D,:] =  operator_6_dag

    Modd[1,:,5,:] = operator_7_dag
    Meven[5,:,D,:] = operator_8_dag
