...
function dostuff(N , D , Ly, coupling, mu; storage="./data/")

    Nlinks = Ly * 2
    Nstates = 2^(Nlinks) # number of states for one chain element

    D_max = 10*D
    A = randmps(N, Nstates, D);
    chemical_potential = [0, mu]
    theta = 0.0
    mpo = mpoqlm(N, Ly = Ly, coupling=coupling , mu=chemical_potential , theta= theta);
    ...
    save_configuration(A, N , D, coupling, theta= theta, mu=chemical_potential, Ly = Ly, prefix =string(storage,"/SQ"))
