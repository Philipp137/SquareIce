...
void totalH_diag(){
  ...
  std::vector<std::vector<double>> evecs;
   ...
   for(i=0;i<NH;i++){
     ...
     */
     ...
     for(j=0;j<=nElem;j++) rowscan[j]=NH;
     stateq = 0;
     for(p=0;p<VOL;p++){
 ...
 void eigcheck(std::vector<double> &evals, std::vector<std::vector<double>> &evecs){
  ...
  ffx = fopen("eigvals_all.dat","w");
  for(i=0;i<NH;i++) fprintf(ffx,"% .12f \n",evals[i]);
  fclose(ffx);
  //for(i=0;i<NH;i++){
  ...
  // this part seems to be causing a segmentation fault, not sure why
  //char DGEMMC = 'T';
  //char DGEMMN = 'N';
