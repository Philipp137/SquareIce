...
void constH(int sector){
   ...
   mLx = LX-2; nElem = LY*mLx;
   std::vector<bool> newstate(2*VOL);
   std::vector<int> rowscan(nElem+1);
   ...
   // basis state = LY*(LX-2). The two extremal plaquettes on either side are frozen due to FBC.
   // In addition, the lambda counts the number of flippable terms in the state.

   // the corrf1(LX/2) has to be modified due to FBC.
   ...
   for(i=0;i<Wind[sector].nBasis;i++){
      ...
      /* a single plaquette is arranged as 
        ...
        pwx  |   p   |  pyz
             ...
             o-------o
                pxy
      */
      for(j=0;j<=nElem;j++) rowscan[j]=NTOT;
      //std::cout<<"State-# ="<<i<<" Buffer element number ="<<rowscan[0]<<std::endl;
      stateq = 0;
      for(p=0;p<VOL;p++){
       // skip the sites with x=0 and x=(LX-1); frozen due to FBC
       x=p%LX;
       if((x==0)||(x==(LX-1))) continue;
       // the other plaquettes can be flipped, and check the new resulting state		       
     ...
     // check
     if(stateq > (nElem+1)){ printf("Error in bound.\n"); }
